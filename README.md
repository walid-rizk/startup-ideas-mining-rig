...
| **Verify** | Two lenses per survivor: **Market Research** (Data Miner validates market size, competitors, customer evidence with **live web search** on Anthropic and Gemini models; emits a Market Confidence rating) and **Stress Test** (Devil's Advocate finds the strongest failure modes; emits a severity rating). A **Run all diligence** button batches both lenses across every survivor. OpenAI models don't have search wired yet — on those, research runs on training knowledge, claims are labeled `Knowledge`/`Proxy`/`Inferred`, and Market Confidence is capped at MODERATE |
