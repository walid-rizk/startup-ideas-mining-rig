...
- **LLM:** Vercel AI SDK (`ai@6.x`) with Anthropic (Haiku 4.5 → Fable 5), Google (Gemini 2.5/3.x), and OpenAI (GPT 5.x) providers. Full list in `MODEL_OPTIONS` (`src/lib/types.ts`). Selected per-session via `SessionToolbar`. Opus 4.7+/Fable 5 reject sampling params — `streamSkill` omits `temperature` for those models (`modelSupportsSampling`; ai@6 omits undefined temperature, unlike ai@3 which defaulted it to 0).
- **Streaming protocol:** routes return `toUIMessageStreamResponse()` (SSE, `data: {json}` events); the client parses it with `streamToText()` (`src/lib/streaming.ts`) — accumulates `text-delta` events, throws on `error` events. Intake uses `useChat` from `@ai-sdk/react` with `DefaultChatTransport`; UIMessages carry `parts`, persisted intake messages stay `{id, role, content}` (mapped both ways in `intake-session.tsx`).
- **Web search:** LIVE for all three providers, wired in `providers.ts` `liveSearchTools()` and used only by the verify route — Anthropic `webSearch_20250305` (NOT `_20260209`, which couples to programmatic tool calling and 400s on Haiku), Gemini Google Search grounding (`google.tools.googleSearch`), OpenAI Responses-API `web_search`. Citations arrive as `source-url` stream events (`sendSources: true`), which `streamToText` collects and appends to the report as a `## Sources` section — Gemini never inlines URLs in text, so this is the only way its citations persist. `modelHasLiveSearch()` in `types.ts` is the central per-provider switch; the verify route and Verify UI both read it. Models removed from it run the data-miner in explicit NO-LIVE-SEARCH mode (no fabricated citations, `Knowledge:`/`Proxy:`/`Inferred:` labels, Market Confidence capped at MODERATE).
...
- **No database:** all state in IndexedDB. Import/export via JSON in `SessionToolbar`.
- **Founder context:** lives in `session.founderContext` (string) in browser IndexedDB — generated by the intake phase, edited on `/intake`. No server-side file.
...
2. **Founder context** — ask the user to paste their Founder Context (or export it from the app's `/intake` page). There's no on-disk context file; the app stores it in browser IndexedDB.
