...
- **Web search:** LIVE for Anthropic (provider-executed `webSearch_20250305` — NOT `_20260209`, which couples to programmatic tool calling and 400s on Haiku) and Gemini (Google Search grounding via `google.tools.googleSearch`), wired in `providers.ts` `liveSearchTools()` and used only by the verify route. `modelHasLiveSearch()` in `types.ts` is the central switch (true for anthropic+gemini, false for openai); the verify route and Verify UI both read it. On non-search models the data-miner runs in explicit NO-LIVE-SEARCH mode (no fabricated citations, `Knowledge:`/`Proxy:`/`Inferred:` labels, Market Confidence capped at MODERATE).
